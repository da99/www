...
var SPLIT_TAG_NAME_VALID_PATTERN = /\A(a-z)([\.\#][a-z0-9\_]+)\Z/;
var SPLIT_TAG_NAME_PATTERN = /([\.\#])/g;
var ALLOWED_TAGS = {
  p: true,
  a: true,
  div: true,
...
function split_tag_name2(new_class) {
  let e = null;
  let curr = "";
...
function element(tag_name, ...pieces) {
  const e = split_tag_name2(tag_name);
  pieces.forEach((x, _i) => {
    if (typeof x === "string")
      return e.appendChild(document.createTextNode(x));
    if (is_plain_object2(x))
...
function element2(tag_name, ...pieces) {
  const eles = [];
  let attrs = undefined;
  pieces.forEach((x, _i) => {
    if (typeof x === "string")
...
function to_html(x) {
  if (typeof x === "string")
    return import_lodash.default.escape(x);
  else
    return x.to_html();
}
class BElement {
  ...
  tagid;
  childs;
  constructor(raw_name, raw_attrs, eles) {
  ...
  to_html() {
    ...
    if (this.classList.length > 0) {
      html += ` class="${this.classList.map((x) => import_lodash.default.escape(x)).join(" ")}"`;
    }
    for (const k in this.attrs) {
      html += ` ${k}="${import_lodash.default.escape(this.attrs[k])}"`;
    }
    html += ">";
    ...
    for (const c of this.childs) {
      if (typeof c === "string")
        html += import_lodash.default.escape(c);
      else
        html += c.to_html();
    }
    html += `</${this.tagname}>`;
...
describe("form_data", function() {
  it("returns an object", function() {
    const data = form_data(element("form", element("input", { name: "msg", type: "hidden", value: "hello" })));
    assert.deepEqual(data, { msg: "hello" });
  });
  it("returns an object with arrays for multiple values", function() {
    const data = form_data(element("form", element("input", { name: "msg", type: "hidden", value: "hello1" }), element("input", { name: "msg", type: "hidden", value: "hello2" })));
...
describe("Build Side Rendering", function() {
  describe("element", function() {
    it("throws if #id is not: a-z 0-9 _", function() {
      ...
      }, "Invalid characters for p id: \"");
      const html = element2('p#hel"lp', "spacer");
    });
    it("throws if .class is not: a-z 0-9 _", function() {
      assert.throws(function() {
      }, "Invalid characters for p class: \"");
      const html = element2('p.he"p', "spacer");
    });
    it(".to_html returns an HTML string", function() {
      const html = element2("span", "hello");
      assert.equal(html.to_html(), "<span>hello</span>");
    });
    it(".to_html escapes tags", function() {
      const html = element2("span", `<script>hello</script>`);
    ...
    it(".to_html renders attributes", function() {
      const html = element2("html", { lang: "en" });
    ...
    it(".to_html escapes quotes in any attribute", function() {
      const html = element2("p", { bob: "hel'lo" });
      assert.equal(html.to_html(), '<p bob="hel&#39;lo"></p>');
    });
  });
  describe("html5", function() {
    it("returns a string with a doctype", function() {
      const html = html5(element2("html", element2("head", element2("title", "hello")), element2("body")));
